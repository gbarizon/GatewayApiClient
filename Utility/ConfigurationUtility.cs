...
namespace GatewayApiClient.Utility {
    ...
    internal class ConfigurationUtility {
        ...
        public static string GetConfigurationId(string configurationId)
        {
            string configurationValue = ConfigurationManager.AppSettings[configurationId];
            if (string.IsNullOrWhiteSpace(configurationValue)) { throw new ConfigurationErrorsException("Missing configuration: " + configurationId); }
